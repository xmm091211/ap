...
//api通道信息
define('API_INFO', "SELECT a_l_id AS l_id,a_l_title AS l_title,a_l_desc AS l_desc,a_l_alias AS l_alias,a_l_address AS l_address,a_l_format AS l_format,a_l_mode AS l_mode,a_l_ask AS l_ask,a_l_demo AS l_demo,a_l_example AS l_example,a_l_data AS l_data ,a_l_show AS l_show ,a_l_found_time AS l_found_time,a_l_count AS l_count,a_l_pay AS l_pay,api_owned.* FROM api_info INNER JOIN api_owned ON api_info.a_l_id = api_owned.ow_l_id WHERE api_info.a_l_alias = ? AND api_owned.ow_md5=?;"); //LIMIT 0, 10000;

//api通道信息2
define('API_INFO2', "SELECT a_l_id AS l_id,a_l_title AS l_title,a_l_desc AS l_desc,a_l_alias AS l_alias,a_l_address AS l_address,a_l_format AS l_format,a_l_mode AS l_mode,a_l_ask AS l_ask,a_l_demo AS l_demo,a_l_example AS l_example,a_l_data AS l_data ,a_l_show AS l_show ,a_l_found_time AS l_found_time,a_l_count AS l_count,a_l_pay AS l_pay FROM api_info WHERE a_l_alias = ?;"); //LIMIT 0, 10000;
