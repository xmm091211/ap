...
class HandleController extends Controllers
{
    ...
    public function addapi()
    {
        ...
        //关键参数
        $val_name = ['title', 'address', 'format', 'mode', 'ask', 'alias', 'target'];
        //判断关键参数是否为空
        if (is_empty($val_name, $val)) {
            //数据打包
            $info = [$val['title'], $val['desc'], $val['alias'], $val['address'], $val['format'], $val['mode'], $val['ask'], $val['demo'], $val['example'],  $val['datatype'], $val['show'], $val['pay']];
            //执行插入
